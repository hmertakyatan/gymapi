...
func (mq *MembershipQuery) QueryCustomer() *CustomerQuery {
...
func (mq *MembershipQuery) Clone() *MembershipQuery {
	...
	return &MembershipQuery{
		...
		withCustomer: mq.withCustomer.Clone(),
...
func (mq *MembershipQuery) WithCustomer(opts ...func(*CustomerQuery)) *MembershipQuery {
	query := (&CustomerClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withCustomer = query
	return mq
}
...
//		GroupBy(membership.FieldOperationDate).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Membership.Query().
//		Select(membership.FieldOperationDate).
//		Scan(ctx, &v)
...
func (mq *MembershipQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Membership, error) {
	...
	if query := mq.withCustomer; query != nil {
		if err := mq.loadCustomer(ctx, query, nodes, nil,
			func(n *Membership, e *Customer) { n.Edges.Customer = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MembershipQuery) loadCustomer(ctx context.Context, query *CustomerQuery, nodes []*Membership, init func(*Membership), assign func(*Membership, *Customer)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Membership)
	for i := range nodes {
		if nodes[i].customer_membership == nil {
			continue
		}
		fk := *nodes[i].customer_membership
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "customer_membership" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
