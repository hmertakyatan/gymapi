...
func (cq *CustomerQuery) QueryMembership() *MembershipQuery {
...
func (cq *CustomerQuery) QueryPersonnelRelCustomer() *PersonnelRelCustomerQuery {
	query := (&PersonnelRelCustomerClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CustomerQuery) Clone() *CustomerQuery {
	...
	return &CustomerQuery{
		...
		withMembership:           cq.withMembership.Clone(),
		withPersonnelRelCustomer: cq.withPersonnelRelCustomer.Clone(),
...
func (cq *CustomerQuery) WithMembership(opts ...func(*MembershipQuery)) *CustomerQuery {
	query := (&MembershipClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withMembership = query
	return cq
}
...
func (cq *CustomerQuery) WithPersonnelRelCustomer(opts ...func(*PersonnelRelCustomerQuery)) *CustomerQuery {
	query := (&PersonnelRelCustomerClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withPersonnelRelCustomer = query
	return cq
}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	var (
		...
		_spec       = cq.querySpec()
		loadedTypes = [2]bool{
			cq.withMembership != nil,
			cq.withPersonnelRelCustomer != nil,
		}
	)
	...
	if query := cq.withMembership; query != nil {
		if err := cq.loadMembership(ctx, query, nodes,
			func(n *Customer) { n.Edges.Membership = []*Membership{} },
			func(n *Customer, e *Membership) { n.Edges.Membership = append(n.Edges.Membership, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withPersonnelRelCustomer; query != nil {
		if err := cq.loadPersonnelRelCustomer(ctx, query, nodes,
			func(n *Customer) { n.Edges.PersonnelRelCustomer = []*PersonnelRelCustomer{} },
			func(n *Customer, e *PersonnelRelCustomer) {
...
func (cq *CustomerQuery) loadMembership(ctx context.Context, query *MembershipQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *Membership)) error {
	...
	query.Where(predicate.Membership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.MembershipColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.customer_membership
		if fk == nil {
			return fmt.Errorf(`foreign-key "customer_membership" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "customer_membership" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (cq *CustomerQuery) loadPersonnelRelCustomer(ctx context.Context, query *PersonnelRelCustomerQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *PersonnelRelCustomer)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Customer)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.PersonnelRelCustomer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.PersonnelRelCustomerColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.customer_personnel_rel_customer
		if fk == nil {
			return fmt.Errorf(`foreign-key "customer_personnel_rel_customer" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "customer_personnel_rel_customer" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
